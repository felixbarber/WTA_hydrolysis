...
expt_labels = ['bFB295_IPTG_induction', 'bFB295_IPTG_induction_Mg_shock']
group_label = 'cwlO_ponA_OE_Mg'
perts = ['IPTG added', r'MgCl$_2$ added']
plot_labels=['IPTG alone', r'MgCl$_2$ treated']
legend_loc=[0.3,0.75]
max_t=95.0

# expt_labels = ['bFB292_IPTG_induction', 'bFB292_IPTG_induction_Mg_shock', 'bFB292_IPTG_induction_Mg_shock_recent']
# group_label = 'lytE_ponA_OE_Mg'
# perts = ['IPTG added', r'MgCl$_2$ added']
# plot_labels=['IPTG alone', r'MgCl$_2$ treated', r'MgCl$_2$ treated recent']
# legend_loc=[0.3,0.75]
# max_t=95.0


# expt_labels = ['bFB292_IPTG_induction_Mg_shock']
# group_label = 'lytE_ponA_OE_Mg'
# perts = ['IPTG added', r'MgCl$_2$ added']
# plot_labels=[r'$\Delta lytE$']
# legend_loc=[0.3,0.75]
# max_t=80.0
...
ylabels = {'sgr_l': r'Growth rate $\lambda_l$', 'sgr_l_pos':r'Growth rate $\lambda_l$', 'sgr_sa': r'Growth rate $\lambda_S$', 'width':'Width', 'width_pos':'Width'}
abbrev_ylabels = {'sgr_l': r'$\lambda_l$', 'sgr_l_pos': r'$\lambda_l$', 'sgr_sa': r'$\lambda_S$', 'width':'Width', 'width_pos':'Width'}
yunits = {'sgr_l': r' $(h^{-1})$', 'sgr_sa': r' ($h^{-1}$)', 'width':r' ($\mu$m)','sgr_l_pos': r' $(h^{-1})$', 'width_pos':r' ($\mu$m)'}
sel_vars=['sgr_l', 'width', 'sgr_l_pos', 'width_pos']
...
with open(out_path + group_label+'.txt', 'w') as f:
    ...
    #
    #         ax=plt.gca()
    #         plt.vlines(0,ymin=ax.get_ylim()[0],ymax=ax.get_ylim()[1],label=perts[0],linestyle=linestyles[0],colors='k')
    #         plt.vlines((expt_vals[-1]['t_pert1'][0]-expt_vals[-1]['t_pert'][0])/60.0, ymin=ax.get_ylim()[0], ymax=ax.get_ylim()[1], label=perts[1], linestyle=linestyles[1],
    #                    colors='k')
        ...
        for var in sel_vars:
            ...
            for expt in expt_labels:
                ...
                if not (max_t is None):
                    temp_vals = np.nonzero(xv/60<max_t)[0][-1]
                else:
                    temp_vals = len(xv)-1
                # xv=xv[:temp_vals] - expt_vals[expt_ind]['t_pert'][-1]
                xv = xv[:temp_vals]
                ...
                err = scipy.ndimage.gaussian_filter(np.nanstd(yv, axis=0), sigma=2)
                # plt.plot(xv / 60, sgr_sa_smoothed, label=plot_labels[expt_labels.index(expt)], lw=0.5)
                plt.plot(xv / 60, sgr_sa_smoothed, lw=0.5, label=plot_labels[expt_labels.index(expt)])
                plt.fill_between(xv / 60, sgr_sa_smoothed - err, sgr_sa_smoothed + err, alpha=0.4)
            ...
            plt.vlines(0,ymin=ymin,ymax=ymax,linestyle=linestyles[0],colors='k')
            plt.vlines((expt_vals[-1]['t_pert1'][0]-expt_vals[-1]['t_pert'][0])/60.0, ymin=ax.get_ylim()[0], ymax=ax.get_ylim()[1], linestyle=linestyles[1],
                       colors='k')
            plt.legend(loc=[1.02,0.1])
            # plt.legend()
            plt.xlabel('Time (min)')
            ...
            plt.ylabel(temp_lab)
            # plt.show()
            # fig.savefig(out_path + group_label + '_' + var + norm + '_compact_xlim.pdf', bbox_inches='tight')
            plt.legend(loc=legend_loc)
            fig.savefig(out_path + group_label + '_' + var + norm + '_compact_legend_xlim.pdf', dpi=300, bbox_inches='tight')
            plt.clf()
